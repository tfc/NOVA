...
void Smmu::fault()
{
    ...
    if (EXPECT_FALSE (fsts & Fault::PPF)) {
        uint64_t hi, lo;
    ...
    if (EXPECT_FALSE (fsts & (Fault::ITE | Fault::ICE | Fault::IQE))) {

        auto const error { read (Reg64::IQERCD) };

        // Invalidation Timeout Error
        if (EXPECT_FALSE (fsts & Fault::ITE)) {
        ...
        if (EXPECT_FALSE (fsts & Fault::ICE)) {
        ...
        if (EXPECT_FALSE (fsts & Fault::IQE))
