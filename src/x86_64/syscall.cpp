...
void Ec::sys_create_ec()
{
    ...
    if (EXPECT_FALSE (r->utcb() >= USER_ADDR || r->utcb() & OFFS_MASK (0) || !pd->insert_utcb (r->utcb()))) {
...
void Ec::sys_assign_pci()
{
    ...
    if (EXPECT_FALSE (!Pd::current->Space_mem::lookup (r->dev(), phys, o) || (rid = Pci::phys_to_rid (phys)) == ~0U)) {
