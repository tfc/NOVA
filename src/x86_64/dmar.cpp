...
Dmar::Dmar (Paddr p) : List<Dmar> (list), reg_base ((hwdev_addr -= PAGE_SIZE (0)) | (p & OFFS_MASK (0))), invq (static_cast<Dmar_qi *>(Buddy::alloc (ord, Buddy::Fill::BITS0))), invq_idx (0)
{
#if 0   // FIXME
    ...
    Dptp::set_mll (static_cast<unsigned>(bit_scan_msb (cap >> 34 & 0xf) + 1));
...
{
    auto lev = static_cast<unsigned>(bit_scan_msb (read<mword>(REG_CAP) >> 8 & 0x1f));
