...
class Console_uart_scif final : private Console_uart
{
    private:
        ...
        void tx (uint8_t c) const override final
        {
            write (Reg8::SCFTDR, c);
            write (Reg16::SCFSR, uint16_t (~BIT_RANGE (6, 5)) & read (Reg16::SCFSR));
        }

        [[nodiscard]] bool tx_busy() const override final { return !(read (Reg16::SCFSR) & BIT (6)); }
        [[nodiscard]] bool tx_full() const override final { return !(read (Reg16::SCFSR) & BIT (5)); }

        [[nodiscard]] bool init() const override final
        {
            write (Reg16::SCSCR, 0);
