...
template<typename T> static constexpr auto min (T v1, T v2)
{
    return v1 < v2 ? v1 : v2;
}

template<typename T> static constexpr auto max (T v1, T v2)
{
    return v1 > v2 ? v1 : v2;
}

template<typename T> static constexpr auto gcd (T v1, T v2)
{
    while (v2) {
        T r { v1 % v2 };
        v1  = v2;
...
template<typename T> static constexpr bool match_list (T const list[], T id)
{
