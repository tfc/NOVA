...
class Lapic final
{
    private:
        ...
        static auto read        (Reg32 r) { return EXPECT_TRUE (x2apic) ? read_x2apic (r) : read_legacy (r); }
        ...
        static void write        (Reg32 r, uint32_t v) { EXPECT_TRUE (x2apic) ? write_x2apic (r, v) : write_legacy (r, v); }
        ...
        static void set_icr (uint64_t v)
        {
            if (EXPECT_TRUE (x2apic))
                ...
                while (EXPECT_FALSE (read_legacy (Reg32::ICR_LO) & BIT (12)))
