...
class Cos final
{
    ...
    private:
        ...
        static void set_cos (cos_t cos) { Msr::write (Msr::Reg64::IA32_PQR_ASSOC, static_cast<uint64_t>(cos) << 32); }

        static void set_l3_mask (uint16_t n, uint32_t val) { Msr::write (Msr::Arr64::IA32_L3_MASK, 1, n, val); }
        static void set_l2_mask (uint16_t n, uint32_t val) { Msr::write (Msr::Arr64::IA32_L2_MASK, 1, n, val); }
        static void set_mb_thrt (uint16_t n, uint16_t val) { Msr::write (Msr::Arr64::IA32_MB_THRT, 1, n, val); }

        static void set_qos_cfg (uint8_t cfg)
        {
            if (supcfg & Config::CDP_L3)    // IA32_L3_QOS_CFG exists
                Msr::write (Msr::Reg64::IA32_L3_QOS_CFG, !!(cfg & Config::CDP_L3) * BIT (0));

            if (supcfg & Config::CDP_L2)    // IA32_L2_QOS_CFG exists
                Msr::write (Msr::Reg64::IA32_L2_QOS_CFG, !!(cfg & Config::CDP_L2) * BIT (0));
        }
         ...
         */
        static bool valid_cbm (uint32_t cbm, uint8_t hcb)
        {
            if (!cbm || cbm > BIT_RANGE (hcb, 0))
                return false;

            auto const c { cbm >> bit_scan_lsb (cbm) };
    ...
    public:
         ...
         * @return      True if valid, false otherwise
         */
        static bool valid_cos (cos_t cos)
        {
            cos_t cos_num { 1 };
            ...
            if (config) {
                ...
                auto const l3 { static_cast<cos_t>(cos_l3 >> !!(config & Config::CDP_L3)) };
                if (cos_num < l3)
                    cos_num = l3;

                // If CDP is enabled, then the valid COS range is half the number of MSRs
                auto const l2 { static_cast<cos_t>(cos_l2 >> !!(config & Config::CDP_L2)) };
                if (cos_num < l2)
                    cos_num = l2;
         ...
         */
        static void make_current (cos_t cos)
        {
         ...
         */
        static auto cfg_qos (uint8_t cfg)
        {
            if (config)
         ...
         */
        static auto cfg_l3_mask (uint16_t n, uint32_t cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static auto cfg_l2_mask (uint16_t n, uint32_t cbm)
        {
            if (!config)
                return Status::ABORTED;
         ...
         */
        static auto cfg_mb_thrt (uint16_t n, uint16_t del)
        {
            if (!config)
                return Status::ABORTED;
